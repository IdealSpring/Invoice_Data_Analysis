...
public class EnterpriesReducer extends Reducer<Text, Enterprise, Enterprise, NullWritable> {
    ...
    protected void reduce(Text key, Iterable<Enterprise> values, Context context) throws IOException, InterruptedException {
        ...
        if(enterprise.isRetain()) {
            ...
            // 19.redInvoiceQuantityRate  红字发票数量占比,22.valueOfRedInovice  红字发票金额
            AttributeUtils.setRedInvoiceQuantityAndAmountRate(enterprise, inputInvoiceSet, outputInvoiceSet);
    ...
    private void cleanUp(Enterprise enterprise) {
        ...
        enterprise.setRedInvoiceQuantityRate("Null");
        enterprise.setNumberDifferenceOfInputAndOutput("Null");
        enterprise.setValueOfRedInovice("Null");
