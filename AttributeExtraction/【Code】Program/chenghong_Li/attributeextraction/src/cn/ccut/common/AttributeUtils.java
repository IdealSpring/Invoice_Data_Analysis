...
import cn.ccut.attribute.Enterprise;
import cn.ccut.attribute.Invoice;
...
public class AttributeUtils {
    ...
    public static void setInvoiceBalance(Enterprise enterprise, TreeSet<Invoice> inputInvoiceSet, TreeSet<Invoice> outputInvoiceSet){
        ...
        // 若进销项有一项不存在, 设值unknown
        if ( inum <= 0 || onum <= 0 || (inum <= 0 && onum <= 0)) {
            enterprise.setInvoiceBalance("unknown");
        }
    ...
    public static void setInputInvoiceInvalid(Enterprise enterprise, TreeSet<Invoice> inputInvoiceSet) {
        ...
        // 作废发票金额
        double invalidInvoiceAmount = 0;

        // 计算总金额和作废金额
        for(Invoice invoice : inputInvoiceSet) {
            String zfbz = invoice.getZfbz();
            invoiceAmount += invoice.getJe();

            if(zfbz.equals("Y")) {
                invalidInvoiceAmount += Math.abs(invoice.getJe());
            }
        }

        if (invoiceAmount == 0 && invalidInvoiceAmount == 0) {
            enterprise.setInputInvoiceInvalid("noInput");
            return;
        } else if (invoiceAmount == 0) {
            enterprise.setInputInvoiceInvalid("onlyInvalid");
            return;
        } else if (invalidInvoiceAmount / invoiceAmount > 0.2) {
            enterprise.setInputInvoiceInvalid("high");
        } else {
            enterprise.setInputInvoiceInvalid("low");
        }
    }
    ...
    public static void setOutputInvoiceInvalid(Enterprise enterprise, TreeSet<Invoice> outputInvoiceSet) {
        ...
        if (invoiceAmount == 0 && invalidInvoiceAmount == 0) {
            enterprise.setOutputInvoiceInvalid("noOutput");
            return;
        } else if (invoiceAmount == 0) {
            enterprise.setOutputInvoiceInvalid("onlyInvalid");
            return;
        } else if (invalidInvoiceAmount / invoiceAmount > 0.2) {
            enterprise.setOutputInvoiceInvalid("high");
        } else {
            enterprise.setOutputInvoiceInvalid("low");
        }
    }
...
/*    public static void setOutputInvoiceInvalid(Enterprise enterprise, TreeSet<Invoice> outputInvoiceSet) {
     ...
     * @param outputInvoiceSet
     */
    public static void setRedInvoiceQuantityAndAmountRate(Enterprise enterprise,
                                                          TreeSet<Invoice> inputInvoiceSet,
                                                          TreeSet<Invoice> outputInvoiceSet) {
		// 发票金额
		double amount = 0;
        ...
        // 计算发票数量和金额
        invoiceNum = invoicesList.size();
		for (Invoice i : invoicesList) {
			double je = i.getJe();
			if (je < 0) {
				redAmount += Math.abs(je);
				redInvoiceNum ++;
            } else if (je >= 0) {
                amount += je;
            }
		...
		// 若不存在红字发票
		if (redInvoiceNum == 0) {
			enterprise.setRedInvoiceQuantityRate("none");
			enterprise.setValueOfRedInovice("none");
			return;
		}

		// 若正常发票金额为0
        if (amount == 0) {
		    enterprise.setRedInvoiceQuantityRate("abnormal");
		    enterprise.setValueOfRedInovice("abnormal");
		    return;
        }

        // 判断红字发票数量
        if ((double) redInvoiceNum / (double) invoiceNum > 0.1) {
		    enterprise.setRedInvoiceQuantityRate("high");
        } else {
		    enterprise.setRedInvoiceQuantityRate("low");
        }

        // 判断红字发票金额
        if ((redAmount / amount) > 0.5) {
            enterprise.setValueOfRedInovice("high");
        } else if ((redAmount / amount) > 0.2) {
            enterprise.setValueOfRedInovice("middle");
        } else {
            enterprise.setValueOfRedInovice("low");
        }
    }
	 ...
	 * @param outputInvoiceSet
	 */
/*	public static void setRedOutputInvoiceAmountRate(Enterprise enterprise, TreeSet<Invoice> outputInvoiceSet) {
		// 发票销项金额
		double outputAmount = 0;
		// 红字发票金额
		double redAmount = 0;
		...
		//若不存在红字发票
		if (redAmount == 0) {
			enterprise.setRedOutputInvoiceAmountRate("none");
		}

		double absRedAmount = Math.abs(redAmount);
		double absInputAmount = Math.abs(outputAmount);
		if ((absRedAmount / absInputAmount) > 0.3) {
			enterprise.setRedOutputInvoiceAmountRate("high");
		} else if ((absRedAmount / absInputAmount) > 0.1) {
