...
public class AttributeUtils {
     ...
     * @param outputInvoiceSet
     */
    public static void setRedInputInvoiceAmountRate(Enterprise enterprise, TreeSet<Invoice> inputInvoiceSet) {
        // 发票进项金额
        double inputAmount = 0;
        // 红字发票金额
        double redAmount = 0;
        ...
        //若不存在红字发票
        if (redAmount == 0) {
            enterprise.setRedInputInvoiceAmountRate("none");
        }

        double absRedAmount = Math.abs(redAmount);
        double absInputAmount = Math.abs(inputAmount);
        if ((absRedAmount / absInputAmount) > 0.3) {
            enterprise.setRedInputInvoiceAmountRate("high");
        } else if ((absRedAmount / absInputAmount) > 0.1) {
            enterprise.setRedInputInvoiceAmountRate("middle");
        } else {
            enterprise.setRedInputInvoiceAmountRate("low");
        }
    }
