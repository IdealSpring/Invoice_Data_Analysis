...
public class DecisionTree {
     ...
     * @return
     */
    private String recusiveSearchClassType(TreeNode node, ArrayList<String[]> remainFeatures) {
        String classType = null;

        try {
            // 如果节点包含了数据的id索引，说明已经分类到底了
            if (node.getDataIndex() != null && node.getDataIndex().size() > 0) {
                classType = treeJudgeClassType(node.getDataIndex());
     ...
     * @return
     */
    public String treeJudgeClassType(ArrayList<String> dataIndex) {
        // 结果类型值
