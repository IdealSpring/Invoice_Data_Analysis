...
import cn.ccut.handle.AnalyzeUtils;
import cn.ccut.handle.Classification;
import cn.ccut.handle.OutputEnterprise;
import cn.ccut.handle.QueryDatabase;
import cn.ccut.pojo.Enterprise;
import cn.ccut.pojo.Invoice;
...
public class AnalyzeReducer extends Reducer<Text, Enterprise, Enterprise, NullWritable> {
    ...
    protected void reduce(Text key, Iterable<Enterprise> values, Context context) throws IOException, InterruptedException {
        ...
        if (enterprise.isRetain() && enterprise.getAbnormal().equals("1")) {
            ...
            // 企业所属行业及注册类型
            QueryDatabase.industryAndType(enterprise);
            ...
            // 按异常个数的多少分类输出企业信息
            OutputEnterprise.outputEnterpriseByAbnormalNum(enterprise);
        }
